...
- Built-in SSH (`--ssh [user@]host[:port]`, or the `[ssh]` table's default connection): one connection per bank, further channels multiplexed over the wire, `tmux -CC` attaching across it, agent-only auth, refuse-by-default `known_hosts` kept read-only, failures printed on the channel's own glass, proven against an in-process russh server: `crates/ssh-link`, `crates/term/src/ssh_channel.rs`, `crates/app/src/ssh.rs`, `docs/ssh.md`. Ceilings: the on-glass chord that picks a target for one channel is #14's; no password, keyboard-interactive, key files or `~/.ssh/config` (a per-row `key` is stored, unread); remote sessions are not auto-discovered. Serves the remote path to P6 and P7; its own market is unsurveyed (see the map below).
