...
The Windows and macOS halves are automatic (`.github/workflows/release.yml`). The Linux half above, the tarball and the `.deb`, is still built and uploaded by hand; carrying questlog's `release-images` pattern the rest of the way would automate that half too. On macOS, signing and notarisation and an Intel or universal build are open.
