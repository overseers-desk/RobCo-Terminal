...
mod tests {
    ...
    /// The exact `_CURRENT_SETTINGS` keys the JSON import format expects, as
    /// a literal checklist independent of whatever fields `GeneralSettings`
    ...
    /// The exact `_CURRENT_SCREEN` keys the JSON import format expects.
    ...
    /// The exact `_CURRENT_CHASSIS` keys the JSON import format expects.
    ...
    fn assert_table_has_keys(table: &toml::Table, keys: &[&str], which: &str) {
        for key in keys {
            assert!(
                table.contains_key(*key),
                "{which} table is missing key `{key}` (expected from the recorded settings checklist)"
            );
        }
        assert_eq!(
            table.len(),
            keys.len(),
            "{which} table has {} keys but the recorded checklist has {}; \
             the struct and the checklist have drifted apart",
            table.len(),
            keys.len()
        );
    }
    ...
    #[test]
    fn general_settings_has_every_stored_key() {
        let toml_string = toml::to_string(&GeneralSettings::default()).unwrap();
        let table: toml::Table = toml::from_str(&toml_string).unwrap();
        assert_table_has_keys(&table, GENERAL_KEYS, "GeneralSettings");
    }

    #[test]
    fn screen_settings_has_every_stored_key() {
        let toml_string = toml::to_string(&ScreenSettings::default()).unwrap();
        let table: toml::Table = toml::from_str(&toml_string).unwrap();
        assert_table_has_keys(&table, SCREEN_KEYS, "ScreenSettings");
    }

    #[test]
    fn chassis_settings_has_every_stored_key() {
        let toml_string = toml::to_string(&ChassisSettings::default()).unwrap();
        let table: toml::Table = toml::from_str(&toml_string).unwrap();
        assert_table_has_keys(&table, CHASSIS_KEYS, "ChassisSettings");
    }
