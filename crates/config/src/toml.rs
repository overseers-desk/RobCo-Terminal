...
///
/// One edit and not two calls to [`write_key`], because the two halves are
/// not separately meaningful. A file that gained a row and no default
/// changed nothing the user asked for; a default naming no row is the stale
/// default `ssh.rs` logs and starts local from. Either half alone is a state
/// nobody chose, and two writes with a power cut between them is exactly how
/// you get one.
///
/// An empty `host` is localhost, which is what `ssh.default = ""` has always
/// spelled: no row is appended and the default is emptied.
///
/// A row the file already carries under the same host, user and port is not
/// duplicated -- only the default moves. Rows differing in any of the three
/// are different destinations and each get their own: `overseer@vault` and
/// `resident@vault` are two accounts, not one row to overwrite.
///
/// This is the one widening of the scalars-only surface above, and it stays
/// inside the same obligations: the row is appended as a real
/// `[[ssh.host]]` table rather than the inline one auto-vivification would
/// leave, nothing else in the file is rewritten, and every byte this did not
/// put there round-trips.
pub fn set_ssh_default_row(
    ...
    edit_document(path, |doc| {
        if !host.is_empty() {
            append_ssh_host(doc, host, user, port, key);
        }
        set_dotted(doc, "ssh.default", toml_edit::Value::from(host));
    })
}

/// The row half of [`set_ssh_default_row`]: append `[[ssh.host]]` unless the
/// destination is already spelled there.
///
/// A `[ssh]` that is not a table (a hand-edited `ssh = 3`, or an inline
/// `ssh = { ... }` this writer cannot append a row inside without rewriting
/// the whole line) is left exactly as it stands, on [`set_dotted`]'s rule:
/// refusing to write is the only edit that cannot destroy what is there.
fn append_ssh_host(doc: &mut DocumentMut, host: &str, user: &str, port: u16, key: &str) {
    let entry = doc
        .as_table_mut()
        .entry("ssh")
        .or_insert(toml_edit::Item::Table(toml_edit::Table::new()));
    let Some(ssh) = entry.as_table_mut() else {
        return;
    };
    // `[ssh]` may have reached the document as the implicit parent of the
    // rows alone, and the default is about to be written into it as a value
    // of its own, which an implicit table has nowhere to show.
    ssh.set_implicit(false);
    let rows = ssh
        .entry("host")
    ...
    let already = rows.iter().any(|row| {
        let text = |name: &str| row.get(name).and_then(|v| v.as_str()).unwrap_or("");
        let number = |name: &str, default: i64| {
            row.get(name).and_then(|v| v.as_integer()).unwrap_or(default)
        };
        text("host") == host && text("user") == user && number("port", 22) == i64::from(port)
    });
    if already {
        return;
    }
    let mut row = toml_edit::Table::new();
    row.insert("host", toml_edit::value(host));
    row.insert("user", toml_edit::value(user));
    row.insert("port", toml_edit::value(i64::from(port)));
    // An empty key is the absent one, and a row saying so in as many words
    // would be this writer's noise in a file the user reads and edits.
    if !key.is_empty() {
        row.insert("key", toml_edit::value(key));
    }
    rows.push(row);
}
...
";
    ...
    // THE PICKER'S ONE WRITE //////////////////////////////////////////////

    /// The commented file the user keeps: it gains a row and a default, and
    /// every other byte of it is the byte it was.
    #[test]
    fn a_typed_destination_gains_a_row_and_the_default_and_costs_no_other_byte() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        let original = "\
# the appliance\n\
...
";
        ...
        set_ssh_default_row(&path, "gw", "resident", 22, "").unwrap();

        let updated = fs::read_to_string(&path).unwrap();
        assert!(
            updated.starts_with("# the appliance\n[general]\nled_characters = 12  # twelve\n"),
            "{updated}"
        );
        assert!(updated.contains("default = \"gw\""), "{updated}");
        assert!(updated.contains("host = \"vault\""), "the row that was there stands");
        assert!(
            updated.contains("[[ssh.host]]\nhost = \"gw\"\nuser = \"resident\"\nport = 22\n"),
            "a real row, not an inline one: {updated}"
        );
        assert!(!updated.contains("key ="), "an absent key is not written");

        // What it reads as: two rows, the typed one the default.
        let config: Config = load(&path).unwrap();
        assert_eq!(config.ssh.default, "gw");
        assert_eq!(config.ssh.hosts.len(), 2);
        assert_eq!(config.ssh.hosts[1].host, "gw");
        assert_eq!(config.ssh.hosts[1].port, 22);
    }

    /// A file that has never heard of SSH: the table appears as a real
    /// `[ssh]` header, the way a hand-written one would be.
    #[test]
    fn a_file_with_no_ssh_table_grows_a_real_one() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        fs::write(&path, "[general]\nled_characters = 12\n").unwrap();

        set_ssh_default_row(&path, "vault", "overseer", 2222, "~/.ssh/id_gw").unwrap();

        let updated = fs::read_to_string(&path).unwrap();
        assert!(updated.contains("[ssh]\n"), "{updated}");
        assert!(updated.contains("default = \"vault\""), "{updated}");
        assert!(updated.contains("[[ssh.host]]"), "{updated}");
        assert!(updated.contains("key = \"~/.ssh/id_gw\""), "{updated}");
        let config: Config = load(&path).unwrap();
        assert_eq!(config.ssh.default, "vault");
        assert_eq!(config.ssh.hosts[0].key, "~/.ssh/id_gw");
    }

    /// The same destination twice: the second time is a default moving, not
    /// a row appearing.
    #[test]
    fn a_destination_the_file_already_names_only_moves_the_default() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        set_ssh_default_row(&path, "vault", "overseer", 2222, "").unwrap();
        set_ssh_default_row(&path, "gw", "resident", 22, "").unwrap();
        let two_rows = fs::read_to_string(&path).unwrap();

        set_ssh_default_row(&path, "vault", "overseer", 2222, "").unwrap();
        let updated = fs::read_to_string(&path).unwrap();
        assert_eq!(
            updated.replace("default = \"vault\"", "default = \"gw\""),
            two_rows,
            "the default is the only thing that moved: {updated}"
        );
        let config: Config = load(&path).unwrap();
        assert_eq!(config.ssh.hosts.len(), 2, "{updated}");
        assert_eq!(config.ssh.default, "vault");

        // A different account on the same host is a different destination.
        set_ssh_default_row(&path, "vault", "someone_else", 2222, "").unwrap();
        let config: Config = load(&path).unwrap();
        assert_eq!(config.ssh.hosts.len(), 3);
    }

    /// Localhost has a spelling already -- an empty default -- and choosing
    /// it appends no row to name.
    #[test]
    fn localhost_is_the_empty_default_and_brings_no_row_with_it() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        set_ssh_default_row(&path, "vault", "overseer", 2222, "").unwrap();

        set_ssh_default_row(&path, "", "", 22, "").unwrap();

        let updated = fs::read_to_string(&path).unwrap();
        assert!(updated.contains("default = \"\""), "{updated}");
        assert_eq!(updated.matches("[[ssh.host]]").count(), 1, "{updated}");
        let config: Config = load(&path).unwrap();
        assert_eq!(config.ssh.default, "");
        assert_eq!(config.ssh.hosts.len(), 1, "the row the user configured stands");
    }

    /// The rows written by hand with no `[ssh]` header above them: the
    /// default needs one, and the file still parses to the same rows.
    #[test]
    fn rows_written_without_their_parent_header_still_take_a_default() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        fs::write(
            &path,
        ...
        let updated = fs::read_to_string(&path).unwrap();
        let config: Config = load(&path).unwrap();
        assert_eq!(config.ssh.default, "gw", "{updated}");
        assert_eq!(config.ssh.hosts.len(), 2, "{updated}");
        assert_eq!(config.ssh.hosts[0].host, "vault");
    }

    /// A hand-edited `ssh` that is not a table: nothing is written, the same
    /// refusal `write_key` makes.
    #[test]
    fn an_ssh_key_that_is_not_a_table_is_left_alone() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        fs::write(&path, "ssh = 3\n").unwrap();
        set_ssh_default_row(&path, "vault", "overseer", 22, "").unwrap();
        assert_eq!(fs::read_to_string(&path).unwrap(), "ssh = 3\n");
    }
