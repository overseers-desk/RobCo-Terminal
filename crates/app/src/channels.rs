...
//!
//! The rules, in full: sessions occupy numbered channel slots on pages. The
//! home page holds the shells this program spawned itself; every tmux -CC
//! attachment is a page of its own, held at channel 1 by the very channel
//! that attached. Within a page, a slot whose shell has exited goes dark and
//! stays dark: no renumbering. New channels take the lowest free slot of
//! their page.
//!
...
//! client itself; that client is [`crate::tmux::Gateway`], held by the
//! surface (`window::TerminalSurface`), and where this model would otherwise
//! call it directly it instead returns a [`Close`] or a `PageId` and lets the
//! surface do it. The transitions themselves are pure, which is what gives
//! [`ChannelKind`] and the held-slot rule their meaning without a wire in the
//! room. One gateway law lives with the surface, not here: the client-size
...
//! other sessions at; the surface's `set_client_size` is that law's one
//! home.
//!
...
pub enum PageKind {
    /// Shells this program spawned itself.
    Home,
...
pub enum ChannelKind {
    /// A PTY shell this program spawned.
    Pty,
    /// A tmux window fed through its page's gateway.
    TmuxPane,
    /// The transported channel standing at a page's slot 1: the one whose PTY
    /// runs `tmux -CC` and carries the page's control stream.
    Gateway,
}
...
pub struct Page {
    ...
    pub tmux_host: String,
    /// The home slot the gateway channel holds while attached. Meaningless on
    /// home.
    pub gateway_home_slot: u32,
...
pub enum Close {
    ...
    CloseWindow,
    /// A gateway channel: detach the page's session. tmux keeps it, the
    /// channel comes home, and the row goes when the detach echoes back.
    Detach(PageId),
...
impl<S> Channels<S> {
    ...
    ///
    /// `page` and `channel` are the sort key and are not the surface's to
    /// write:
    /// where those move, the transition that moves them ([`Self::move_current_to`],
    /// [`Self::attach`], [`Self::collapse_page`]) re-sorts, and a write
    /// here would not.
    ...
    /// it is attached; the hold lives on the page, not on any row.
    fn slot_held(&self, channel: u32) -> bool {
        self.pages
            .iter()
            .any(|p| p.kind == PageKind::Tmux && p.gateway_home_slot == channel)
    }
    ...
    pub fn first_free(&self, page: PageId) -> u32 {
        ...
        if page == 0 {
            for p in self.pages.iter().filter(|p| p.kind == PageKind::Tmux) {
                if p.gateway_home_slot <= CHANNEL_CAP {
                    taken[p.gateway_home_slot as usize] = true;
                }
            }
        }
    ...
    /// and turning it to another page no less; re-selecting the
    ...
    /// `:213-225`. PTY shells live on home alone: an attachment's channels
    ...
    /// `new_window_pending` flag, or `None` when the shell was opened here.
    pub fn new_channel(&mut self, session: impl FnOnce() -> Option<S>) -> Option<PageId> {
        ...
        match self.page_row_of(view) {
            Some(i) if self.pages[i].kind == PageKind::Tmux => {
                self.pages[i].new_window_pending = true;
                Some(view)
            }
            _ => {
    ...
    pub fn new_tmux_window(&mut self) -> Option<PageId> {
    ...
    /// slot of its page, from 2 up: the gateway holds 1. Windows the attach
    ...
    pub fn open_tmux_pane(
        ...
        let asked = self.pages[page_row].new_window_pending;
        if asked {
            self.pages[page_row].new_window_pending = false;
        }
        // An attach's first window also takes the air: the user who typed
        // `tmux -CC` meets a live prompt, the gateway channel standing
        // by at slot 1, not by the gateway's own frozen glass.
        //
        // "First" is the attachment's own count (`Page::attach_done`) and not
        // the slot number. Written as `channel == 2` it was a positional test
        // that fires again every time slot 2 falls vacant: close the window
        // standing there, land back on the gateway, and the next window another
        // client volunteers -- one this set never asked for -- would take the
        // air off the gateway as if it were the greeting.
        let greets = !self.pages[page_row].attach_done
            && self.current_page == page
        ...
        self.pages[page_row].attach_done = true;
        self.insert_row(Row {
            page,
            channel,
            title: normalize_title(name),
            kind: ChannelKind::TmuxPane,
            tmux_window: window_id.to_string(),
    ...
    pub fn close_channel(&mut self, page: PageId, channel: u32) -> Close {
        ...
        match self.rows[index].kind {
            ChannelKind::Gateway => Close::Detach(page),
            ChannelKind::TmuxPane => Close::KillWindow {
                page,
                window_id: self.rows[index].tmux_window.clone(),
            },
            ChannelKind::Pty => {
                if self.rows.len() <= 1 {
    ...
    /// ordinary end of a channel; for a gateway channel it is the tmux client
    /// dying under the session, and the page collapses.
    pub fn session_died(&mut self, page: PageId, channel: u32) -> Close {
        let Some(index) = self.row_of(page, channel) else {
            return Close::Nothing;
        };
        if self.rows[index].kind == ChannelKind::Gateway {
            return self.gateway_died(page);
        }
        if self.rows.len() <= 1 {
            return Close::CloseWindow;
        }
    ...
    /// `:366-379`. The gateway channel's own shell died: collapse the page as a
    /// detach would, which lands its row home as a PTY channel, then remove
    /// that row, there being no live process to come home to.
    fn gateway_died(&mut self, page: PageId) -> Close {
        let Some(page_row) = self.page_row_of(page) else {
            return Close::Nothing;
        };
        let gateway_home_slot = self.pages[page_row].gateway_home_slot;
        self.collapse_page(page);
        self.remove_row(0, gateway_home_slot);
    ...
    pub fn move_current_to(&mut self, page: PageId, channel: u32) -> bool {
        // The bank can be viewing one page while another holds the air, and a
        // store chord lands on the page on view: there is no slot on it the
        // session could take without leaving its own page. Nothing happens,
        ...
        if self.rows[from].kind == ChannelKind::Gateway {
            return false;
        }
        ...
        if to.is_some_and(|i| self.rows[i].kind == ChannelKind::Gateway) {
            return false;
        }
    ...
    /// `:484-499`. Cycling walks the current page: the other pages' channels
    ...
    /// owns it, and a tmux pane's title is tmux's to give (`:750-754`).
    ...
    /// channel transports to slot 1 of a new page, titled for the host the
    ...
    /// holds steady. Returns the new page's id.
    pub fn attach(&mut self, page: PageId, channel: u32, tmux_host: &str) -> Option<PageId> {
        let index = self.row_of(page, channel)?;
        if self.rows[index].kind != ChannelKind::Pty {
            return None;
        }
        ...
        self.pages.push(Page {
            id: page_id,
            kind: PageKind::Tmux,
            tmux_host: tmux_host.to_string(),
            gateway_home_slot: channel,
            new_window_pending: false,
            attach_done: false,
        });
        let was_current = page == self.current_page && channel == self.current_channel;
        ...
        self.rows[index].kind = ChannelKind::Gateway;
        self.rows[index].title = gateway_title(tmux_host);
        self.resort();
        if was_current {
    ...
    /// `:577-608`. Detach or gateway death: the page's pane rows vanish, the
    /// gateway channel transports home to the slot it never gave up and
    /// relights, and the user lands on it.
    pub fn collapse_page(&mut self, page: PageId) {
        ...
        self.degauss_armed = false;
        self.rows
            .retain(|r| !(r.page == page && r.kind == ChannelKind::TmuxPane));
        if let Some(gateway) = self.rows.iter().position(|r| r.page == page) {
            self.rows[gateway].page = 0;
            self.rows[gateway].channel = gateway_home_slot;
            self.rows[gateway].kind = ChannelKind::Pty;
            self.resort();
        }
        ...
        self.set_current(0, gateway_home_slot);
        self.degauss_armed = armed;
    }

    /// `:669-681`. The tmux server's hostname can resolve after the handshake:
    /// the page and its gateway row's title follow it.
    pub fn tmux_host_changed(&mut self, page: PageId, tmux_host: &str) {
        let Some(page_row) = self.page_row_of(page) else {
            return;
        };
        self.pages[page_row].tmux_host = tmux_host.to_string();
        let title = gateway_title(tmux_host);
        for row in self.rows.iter_mut() {
            if row.page == page && row.kind == ChannelKind::Gateway {
    ...
    pub fn channel_of_window(&self, page: PageId, window_id: &str) -> u32 {
        self.rows
            .iter()
            .find(|r| {
                r.page == page && r.kind == ChannelKind::TmuxPane && r.tmux_window == window_id
            })
            .map(|r| r.channel)
            .unwrap_or(0)
    }
...
/// `:547`. What a gateway channel's row reads while it is attached.
fn gateway_title(tmux_host: &str) -> String {
    format!("tmux -CC # @{tmux_host}")
}
...
mod tests {
    ...
    fn an_attachments_windows_fill_from_slot_two_and_the_first_takes_the_air() {
        ...
        assert!(set.open_tmux_pane(page, "@1", "%1", "vim", || Some(1)));
        assert_eq!(
        ...
        assert!(set.open_tmux_pane(page, "@2", "%2", "logs", || Some(2)));
        assert_eq!(set.current_channel(), 2);
        assert_eq!(set.slot_title(page, 3), Some("logs"));
        // ...but one this set asked for takes it outright.
        assert_eq!(set.new_tmux_window(), Some(page));
        assert!(set.open_tmux_pane(page, "@3", "%3", "asked", || Some(3)));
        assert_eq!(set.current_channel(), 4);
    }
    ...
    fn a_volunteered_window_landing_back_on_slot_two_does_not_take_the_air() {
        ...
        assert!(set.open_tmux_pane(page, "@2", "%2", "volunteered", || { Some(2) }));
    ...
    fn a_collapse_brings_the_gateway_home_to_the_slot_it_never_gave_up() {
        let mut set = channels();
        open(&mut set, 0, 5, 55);
        let page = set.attach(0, 5, "prime").unwrap();
        ...
        assert_eq!(set.current().unwrap().kind, ChannelKind::Pty);
        assert_eq!(set.len(), 2, "the pane went with the page");
        assert_eq!(set.pages().len(), 1);
    }

    #[test]
    fn a_gateways_death_collapses_the_page_and_takes_the_returned_row_with_it() {
        let mut set = channels();
        open(&mut set, 0, 2, 22);
        let page = set.attach(0, 2, "prime").unwrap();
        set.open_tmux_pane(page, "@1", "%1", "vim", || Some(1));
    ...
    #[test]
    fn a_store_never_crosses_pages_and_a_gateway_never_moves() {
        let mut set = channels();
        let page = set.attach(0, 1, "prime").unwrap();
        set.open_tmux_pane(page, "@1", "%1", "vim", || Some(1));
    ...
    #[test]
    fn a_close_of_a_pane_row_is_tmuxs_to_do_and_the_row_stays_until_it_says() {
        let mut set = channels();
        let page = set.attach(0, 1, "prime").unwrap();
        set.open_tmux_pane(page, "@7", "%7", "vim", || Some(1));
        assert_eq!(
    ...
    fn ctrl_shift_t_acts_on_the_page_the_bank_is_showing() {
        let mut set = channels();
        let page = set.attach(0, 1, "prime").unwrap();
        // The air is on an attachment, but the bank has stepped back to home:
    ...
    fn the_tmux_host_can_resolve_after_the_handshake() {
        let mut set = channels();
        let page = set.attach(0, 1, "").unwrap();
        assert_eq!(set.current_title(), Some("tmux -CC # @"));
        set.tmux_host_changed(page, "prime");
        assert_eq!(set.current_title(), Some("tmux -CC # @prime"));
    }
}
