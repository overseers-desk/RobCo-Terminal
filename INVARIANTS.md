...
- The user meets nothing but the simulated glass unless they ask to leave it: every runtime prompt, picker, progress line, error and secret is text on the terminal's own grid, typed through its own keyboard path. A native surface (the settings window or any other) appears only on an explicit bail-out gesture: a right-click, a chord bound to that purpose, or the platform's own application menu where it draws one; it may hold configuration, and never holds a password, a passphrase, or a connect-time question.
